...
public class StatutoryDescriptionIdUpdater extends IdUpdater {
  ...
  private void discoverIds() {
    ...
    ApiResponse<V3VmOpenApiGuidPage> response = ptvApi.getGeneralDescriptionApi().apiV4GeneralDescriptionGet(null, page);
    if (response.isOk()) {
      V3VmOpenApiGuidPage pageData = response.getResponse();
      List<String> ids = getItemListIds(pageData.getItemList());
      
      updateRequest.fire(new StatutoryDescriptionIdUpdateRequest(ids, false));
  ...
  private void discoverPriorityIds() {
    ...
    ApiResponse<V3VmOpenApiGuidPage> response = ptvApi.getGeneralDescriptionApi()
        .apiV4GeneralDescriptionGet(priortyScanTime, 0);
    if (response.isOk()) {
      V3VmOpenApiGuidPage pageData = response.getResponse();
      List<String> ids = getItemListIds(pageData.getItemList());
      
      updateRequest.fire(new StatutoryDescriptionIdUpdateRequest(ids, true));
      discoverCount += ids.size();
